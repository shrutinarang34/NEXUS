...
export async function generateFinancialInsights(input: GenerateInsightsInput): Promise<GenerateInsightsOutput> {
  // Hotfix: Filter out persons with invalid currentBalance to prevent AI flow from crashing.
  ...
  prompt: `
    ...
    **Instructions:**
    ...
    1.  **Analyze Expenses & Budgets:**
        ...
        *   When mentioning monetary values, use the currency code (e.g., "100 {{currency}}").
    ...
    2.  **Analyze Accounts, Net Worth & Cash Flow:**
        ...
        *   When mentioning monetary values, use the currency code (e.g., "5000 {{currency}}").
